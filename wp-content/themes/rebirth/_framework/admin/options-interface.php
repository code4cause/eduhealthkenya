		...
		case "multicheckHomepageBlocks":
			...
			$output .= '<div class="homepage-block header no-sorting">Header</div>';
				foreach ($value['options'] as $key => $option) {
					...
					if ($key == 'widget_block'){
						$output .= '<div id="homepageBlock-widget-block" class="homepage-block widget-block">'.esc_html( $option ).'</div>';
					} else {
						$checked = '';
						$label = $option;
						$option = preg_replace('/\W/', '', strtolower($key));
						...
						if ($key == 'js_footer_mapblock'){ $no_sorting = ' no-sorting'; } else { $no_sorting = ''; }
						
						$output .= '<div id="' . esc_attr( $id ) . '" class="homepage-block ' . esc_attr( $id ) . $no_sorting .'">';
						$output .= '<input id="' . esc_attr( $id ) . '" class="checkbox of-input" type="checkbox" name="' . esc_attr( $name ) . '" ' . $checked . ' /><label for="' . esc_attr( $id ) . '">' . esc_html( $label ) . '</label><br />';
						$output .= '</div>';
					}
				}
			$output .= '<div class="homepage-block footer no-sorting">Footer</div>';
			$output .= '</div>';
		break;
